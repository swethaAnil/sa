...
<h3>
<ul>
<li>Changes made here are written to a working copy of the schedule (orders_dev table).</li>
<li>Changes must be published to the Current_Orders table in order to appear on the "Current_Schedule" page.</li>
</ul>
</h3>
<FORM>
...
$curr_date = NULL;
$acDate_output = "SELECT activation_date FROM orders_dev ORDER BY activation_date";
$acDate_result = mysql_query($acDate_output,$connection);

while($row = mysql_fetch_array($acDate_result)){

	if($row['activation_date'] != $curr_date){
		$acDate[] = $row['activation_date'];
		$curr_date = $row['activation_date'];
	}	
}

if(count($acDate) > 1){

	$alertMsg = "ERROR!  Orders in orders_dev table show multiple activation dates (";
	foreach($acDate as $thisDate){
		$alertMsg .= "$thisDate, ";
	}
	$alertMsg = substr($alertMsg,0,-2);
	$alertMsg .= ")";
	echo "<h1 style=\"color:red;\">" . $alertMsg . "</h1>";
	exit;

}else{

	echo "<h2>ACTIVATION DATE: $acDate[0]</h2>";
...
if (mysql_num_rows($result_techcount) > 0) {

	while($tech = mysql_fetch_array($result_techcount)){
		if($tech[0] != "1000"){
			$id_array[] = $tech[0];
			$count_array[] = $tech[1];
		}
	...
	if(isset($id_array)){
		echo "<b>Order Counts</b><table><tr>";
...
if (isset($num_techs) && $num_techs > 0) {
