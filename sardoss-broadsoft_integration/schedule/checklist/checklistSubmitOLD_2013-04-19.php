...
if($source == "v2_order_dashboard"){
	$questions_complete_SQL = "SELECT * FROM checklist_data WHERE answer != '0' AND order_num = '$order_num' AND qID BETWEEN 10037 AND 10066;";
	...
	$update_percentage_SQL = "UPDATE current_v2_orders SET checklist_percentage = '$complete_percentage' WHERE order_num = '$order_num';";
}else{
	$questions_complete_SQL = "SELECT * FROM checklist_data WHERE answer != '0' AND order_num = '$order_num' AND (qID < 10037 OR qID > 10066);";
	$questions_complete_result = mysql_query($questions_complete_SQL,$connection);
	$num_questions_complete = mysql_num_rows($questions_complete_result);
	$complete_percentage = round(($num_questions_complete/$_POST['question_count']) * 100,0);
