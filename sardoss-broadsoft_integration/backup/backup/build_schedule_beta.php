...
function get_tech($qualified_techs,$slot){
	...
	foreach($qualified_techs as $qualified_tech){
		...
		$sql_output = "SELECT * FROM current_orders_beta WHERE tech_id = '$qualified_tech'";
...
//------FILTER OUT TECHS WITH TOO MANY OVERALL ORDERS
		if($tech_order_count < $orders_per_tech){
			...
			$sql_output = "SELECT * FROM current_orders_beta WHERE tech_id = '$qualified_tech' AND timeslot = '$slot'";
...
//--FIND TECHS WITH FEWEST OVERALL ORDERS
	foreach($remaining_techs as $remaining_tech){
		...
		$sql_output = "SELECT * FROM current_orders_beta WHERE tech_id = '$remaining_tech'";
		$tech_all_orders = @mysql_query($sql_output,$connection);
		$tech_order_count = mysql_num_rows($tech_all_orders);
		...
		$sql_output = "SELECT * FROM current_orders_beta WHERE tech_id = '$remaining_tech' AND timeslot = '$slot'";
		$tech_slot_orders = @mysql_query($sql_output,$connection);
		$tech_slot_count = mysql_num_rows($tech_slot_orders);
		$count = 1;
...
$num_orders = uncondition_count('current_orders_beta');
...
$database = @mysql_select_db($databasename,$connection) or die(mysql_error());
$sql_output = "SELECT timeslot, COUNT(*) FROM current_orders_beta GROUP BY timeslot";
$sql_result = @mysql_query($sql_output,$connection);

echo "<b>Schedule Outlook</b><table border=\"1\"><tr><th class=\"coverage\">Timeslot</th><th class=\"coverage\">Orders</th></tr>";
while ($row = mysql_fetch_array($sql_result, MYSQL_NUM)){
	echo "<tr><td class=\"coverage\">" . $row[0] . "</td><td class=\"coverage\">" . $row[1] . "</td></tr>";
}
echo "</table>";
...
$sql_output = "UPDATE current_orders_beta SET tech_id = NULL";
mysql_query($sql_output,$connection);

do{
	//RETRIEVE ALL ORDER INFO SORTED BY TIMESLOT
	$sql_output = "SELECT * FROM current_orders_beta ORDER BY timeslot";
	$sql_result = @mysql_query($sql_output,$connection);
	...
	while ($row = mysql_fetch_array($sql_result, MYSQL_ASSOC)){
		...
		$sql_input = "UPDATE current_orders_beta SET tech_id = '$tech_id', tech_info =  WHERE instance_id = '$instance_id'";
	...
	$sql_output = "SELECT tech_id, COUNT(*) FROM current_orders_beta GROUP BY tech_id";
	$sql_result = @mysql_query($sql_output,$connection);
...
if($stddev > $target_SD){
	echo "<br><br>FAILED TO MEET TARGET STANDARD DEVIATION ($target_SD).<br>Lowest standard deviation: " . $lowest_std_dev . "<br>Overflows: " . $overflow_count;
}elseif($stddev <= $target_SD){
	echo "<br><br>TARGET STANDARD DEVIATION ($target_SD) MET.<br>Standard deviation: " . $lowest_std_dev . "<br>Overflows: " . $overflow_count;
}
...
<FORM>
<INPUT TYPE="BUTTON" VALUE="View/Export Schedule" ONCLICK="window.location.href='view_schedule.php'">
