...
function build_checklist($order_num,$connection,$source){
	...
	if($source == "v2_order_dashboard"){
		...
		$get_percentage_SQL = "SELECT checklist_percentage FROM current_v2_orders WHERE order_num = '$order_num'";
		$title = "2.0 PRE-CHECKS CHECKLIST";
	}else{
		$get_percentage_SQL = "SELECT checklist_percentage FROM current_orders WHERE order_num = '$order_num'";
		...
		$title = strtoupper($order_data['order_type']) . " ORDER CHECKLIST";
		$item_list = NULL;
		$question_count = 0;
		$checklist_html = NULL;
	...
	if($checklist_structure[0]){
		...
		return array($checklist_questions_result,$get_percentage_SQL,$title);
		
	}else{
		echo "<br><b>There is no checklist for order type: \"$order_data[order_type]\"</b><br><br>";
		exit();
	}
}
