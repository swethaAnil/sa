...
<H3>This data has been uploaded to the TCPS Order Database ('tcps_orders')</H3>
<INPUT TYPE="BUTTON" VALUE="Back to TCPS Order Manager" ONCLICK="window.location.href='./'"></FORM>
<br><br>
...
if($error != 1){  //ERROR FOR NO DATE 
	...
	foreach( $data as $row ) { 
		...
		$SQL_find_order = "SELECT * FROM tcps_orders WHERE order_num = '$order_num'";
		...
		if($order_count == 1){
			...
			$SQL_update = "UPDATE tcps_orders SET siebel_order_status = '$status', bandwidth = '$bandwidth', upload_date = '$upload_date',activation_date = '$activation_date',access_type = '$access_type',order_type = '$order_type',package = '$package',timeslot = '$timeslot' WHERE order_num = '$order_num'";
			$update_result = mysql_query($SQL_update);
			$result = "UPDATED";
		}elseif($order_count == 0 ){
			//define SQL full insert query
			$SQL_insert = "INSERT IGNORE INTO tcps_orders (siebel_order_status, bandwidth, v2_tech_id, upload_date, activation_date, acct_name, acct_num, access_type, order_num, order_type, market, package, timeslot) VALUES ('$status', '$bandwidth', 1000,'$upload_date','$activation_date','$acct_name','$acct_num','$access_type','$order_num','$order_type','$market','$package','$timeslot')";
